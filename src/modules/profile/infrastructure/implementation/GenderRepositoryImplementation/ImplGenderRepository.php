...
namespace Src\modules\profile\infrastructure\implementation\GenderRepositoryImplementation;
...
class ImplGenderRepository implements GenderRepositoryInterface
{
    ...
    {
        try {
            $genders = CtlGender::orderBy('id')->paginate($per_page);
            $data = array_map(fn($item) => $this->mapToDomain($item), $genders->items());
